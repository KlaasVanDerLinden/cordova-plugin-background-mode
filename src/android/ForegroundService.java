...
public class ForegroundService extends Service {
    ...
    private void keepAwake() {
        ...
        if (!isSilent) {
            if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
                ...
                nm.deleteNotificationChannel(NOTIFICATION_CHANNEL_ID_SERVICE);

                Integer importance = NotificationManager.IMPORTANCE_LOW;

                NotificationChannel channel =  new NotificationChannel(NOTIFICATION_CHANNEL_ID_SERVICE, "App Service",
                        importance);
        ...
        wakeLock = powerMgr.newWakeLock(
                PowerManager.PARTIAL_WAKE_LOCK, "app:BackgroundMode");
        wakeLock.acquire();
        */
    }
    ...
    private Notification makeNotification(JSONObject settings) {
        ...
        if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.O) {
            notification = new Notification.Builder(context, NOTIFICATION_CHANNEL_ID_SERVICE);
        }
        else{
        ...
        notification.setContentTitle(title)
                .setContentText(text)
                .setOngoing(true);
        notification.setSmallIcon(getIconResId(settings, "small"));
        if(Build.VERSION.SDK_INT >= Build.VERSION_CODES.LOLLIPOP){
            Integer resId = getIconResId(settings, "large");
            if(resId != 0) {
                notification.setLargeIcon(BitmapFactory.decodeResource(getResources(), resId));
            }
        }
     ...
     */
    private int getIconResId(JSONObject settings, String size) {
        Context context = getApplicationContext();
        Resources res   = context.getResources();
        String pkgName  = context.getPackageName();
        String iconKind = "icon";
        String fallback = NOTIFICATION_ICON;
        if(size == "large"){
            iconKind = "icon_large";
            fallback = "";
        }
        String icon     = settings.optString(iconKind, fallback);
    ...
    private int getIconResId(Resources res, String icon,
        ...
        if(resId == 0) {
            // See if we can get it from android.R.drawable, e.g. android.R.drawable.ic_media_pause
            try {

                Field field = android.R.drawable.class.getField(icon);
                try {
